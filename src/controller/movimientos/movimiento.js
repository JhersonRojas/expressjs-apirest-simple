    ...
    conexion.query(sql, async (error,datos) => {
         if ( datos.length > 0) {
             ...
             conexion.query(sqla,async (error,datos) => { 
                await resp.send({ boolean: "true",  datos: datos, mensaje : "Ha cambiado los datos de la reserva de la biblioteca"} )
             })

         } else {
             resp.json({ mensaje : "Lo siento, este reserva no existe", boolean : "false"})
         }
        })
    }catch(error) {
        resp.send(error)

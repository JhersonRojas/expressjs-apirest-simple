...
controlador.acceso= (req,resp) => {
   ...
   let sql = `select id_usuario, Nombre,tipo_usuario, clave from usuarios where id_usuario = ${user} and clave = '${pass}'`

   conexion.query(sql,(error,datos) => {
       if ( datos == "" ) resp.json(`Acceso denegado`)
       else resp.send(datos); console.log(datos.length)
   })
};

    // <------------------ Metodo post para hallar un usuario ---------------->
...
controlador.token = (req, res, nex) => {
    ...
    const decoded = jwt.verify(tokenUsuario,process.env.AUTH_SECRET, (error, decoded) => {
        if(error) return res.json({autorizado: false, mensaje: 'El token no es correcto'});
        else nex()
    })
};
